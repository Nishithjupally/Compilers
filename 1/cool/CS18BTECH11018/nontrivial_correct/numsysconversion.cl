class Main inherits IO {
    ...
    powers(x:Int ,y:Int):Int{ --computes x^y
        if(y=0) then 1
        else x*powers(x,y-1)
        fi
    };

    binary(x : Int, y: Int):Int{{ --binary to decimal
        while(r<x) loop{
            b <- modf(x,10);
            x <- x/10;
            y <- y + b*powers(2,i2);
    ...
    decimal(p : Int,q:Int):Int{{ --decimal to binary
        while(r<p) loop{
            b1 <- modf(p,2);
            p <- p/2;
            q <- q + b1*i1;
    ...
    hexa(p1:Int,q1: String):String{{ --decimal to hexadecimal
        while(r<p1) loop{
            b1 <- modf(p1,16);
            p1 <- p1/16;
            if(b1 = 0) then{
    ...
    hexadeci(s : String, ans : Int):Int{{ --hexadecimal to decimal
        temp <- s.length()-1;
        while(r<=temp) loop{
    ...
    main() : Object { --for operation and input from user
        {
