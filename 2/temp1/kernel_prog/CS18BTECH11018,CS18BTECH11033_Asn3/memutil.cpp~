	...
	while (1) {
		...
		case 'p':
			...
			if (strcmp("demand", optarg) == 0) {
				mmap_flags = MAP_SHARED;	//demand paging flags  
			} else if (strcmp("prefetch", optarg) == 0) {
				mmap_flags = MAP_POPULATE | MAP_SHARED;	//prefetching flags     
			} else {
		...
		case OP_MAPREAD:{
				...
				if (msg_len > 0) {
					for (int i = 0; i < MYDEV_LEN; i++) {
						//checking if we made error in writing
						if (*(dev_mem + i) !=
						    *(msg + (i % msg_len))) {
							flag = 1;
							break;
				...
				if (flag == 1) {
					return EXIT_FAILURE;	//if error in writing
				}
				break;
			}
		...
		case OP_MAPWRITE:{
				...
				dev_mem =
				    (char *)mmap(NULL, MYDEV_LEN, PROT_WRITE,
						 mmap_flags, dev_fd, 0);
				for (int i = 0; i < MYDEV_LEN; i++) {
					*(dev_mem + i) = *(msg + (i % msg_len));	//write message to data         
				}
				munmap(dev_mem, MYDEV_LEN);
				break;
			}
